...
std::tr1::shared_ptr<TcpListenSocket> createListenSocket(
...
TEST(TCPTest, ListenTest) {
  ...
  em.start(4);
  int port = 0;
  std::tr1::shared_ptr<TcpListenSocket> s = createListenSocket(&em, port);
  ASSERT_TRUE(s != NULL);
}
...
TEST(TCPTest, ListenConnectTest) {
  ...
  std::tr1::shared_ptr<TcpSocket> c = TcpSocket::connect(&em, "127.0.0.1", port);
  ASSERT_TRUE(s);
  ASSERT_TRUE(c);
  ...
  for (int i = 1; i < 4 && !found; i++) {
    std::tr1::shared_ptr<TcpSocket> c2 = TcpSocket::connect(&em, "127.0.0.1", port+i);
    if (!c2) {
      found = true;
    }
  }
...
void acceptHandler(Notification *n, string expected, std::tr1::shared_ptr<TcpSocket> *ps, std::tr1::shared_ptr<TcpSocket> s) {
  *ps = s;
...
TEST(TCPTest, ListenConnectSendDataTest) {
  ...
  Notification n;
  std::tr1::shared_ptr<TcpSocket> ps;
...
void acceptDisconnectHandler(Notification *n, std::tr1::shared_ptr<TcpSocket> *ps, std::tr1::shared_ptr<TcpSocket> s) {
  *ps = s;
...
void acceptConnectionCallback(std::tr1::shared_ptr<TcpSocket> s) {
...
TEST(TCPTest, WriteBeforeStartedClient) {
  ...
  std::tr1::shared_ptr<TcpSocket> t(TcpSocket::connect(&em, "127.0.0.1", port));
  Notification n;
...
void FillWriteBufferAccept(int *counter, Notification *n, std::tr1::shared_ptr<TcpSocket> *ps, std::tr1::shared_ptr<TcpSocket> s) {
  *ps = s;
...
TEST(TCPTest, FillWriteBuffer) {
  ...
  int counter = 0;
  Notification n;
  std::tr1::shared_ptr<TcpSocket> accept_sock;
